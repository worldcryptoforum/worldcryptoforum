...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("WCFMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("WCFMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("WCFMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("worldcryptoforum-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("WCFMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running WCFMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadWCFMiner() exception");
    } catch (...) {
        LogPrintf("ThreadWCFMiner() exception");
    }

    LogPrintf("ThreadWCFMiner exiting\n");
}
