...
#define URI_SCHEME "worldcryptoforum"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a WCF address (e.g. %1)").arg("WR2NvWs9gY7sPywPexMEycQCDSqxGQjUq8"));
#endif
...
{
    // return if URI is not valid or is no WCF: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::WCF, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert worldcryptoforum:// to worldcryptoforum:
    //
    //    Cannot handle this later, because worldcryptoforum:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::WCF, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open worldcryptoforum.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "WCF.lnk";
}
...
{
    return GetAutostartDir() / "worldcryptoforum.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a worldcryptoforum.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the worldcryptoforum app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add worldcryptoforum app to startup item list
