...
CPubKey CWallet::GenerateNewKey()
{
    ...
    bool fCompressed = CanSuworldcryptoforumrtFeature(FEATURE_COMPRPUBKEY); // default to compressed public keys if we want 0.6.0 wallets
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-worldcryptoforum-address
                    CScript scriptChange;
    ...
    BOOST_FOREACH (PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins) {
        ...
        if (CheckStakeKernelHash(nBits, block, *pcoin.first, prevoutStake, nTxNewTime, nHashDrift, false, hashProofOfStake, true)) {
            ...
            if (whichType != TX_PUBKEY && whichType != TX_PUBKEYHASH) {
                if (fDebug && GetBoolArg("-printcoinstake", false))
                    LogPrintf("CreateCoinStake : no suworldcryptoforumrt for kernel type=%d\n", whichType);
                break; // only suworldcryptoforumrt pay to public key and pay to address
            }
