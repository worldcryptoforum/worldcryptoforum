...
string HTTPPost(const string& strMsg, const map<string, string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: worldcryptoforum-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
string HTTPError(int nStatus, bool keepalive, bool headersOnly)
{
    if (nStatus == HTTP_UNAUTHORIZED)
        return strprintf("HTTP/1.0 401 Authorization Required\r\n"
                         "Date: %s\r\n"
                         "Server: worldcryptoforum-json-rpc/%s\r\n"
...
/**
 * JSON-RPC protocol.  WCF speaks version 1.0 for maximum compatibility,
