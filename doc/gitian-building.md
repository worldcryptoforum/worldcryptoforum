...
*Setup instructions for a gitian build of WCF using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the WCF
...
to worldcryptoforum-crypto.com.
...
- [Building WCF](#building-worldcryptoforum)
...
Clone the git repositories for worldcryptoforum and gitian and then checkout the worldcryptoforum version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the worldcryptoforum repository
...
To build WCF (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the worldcryptoforum repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/worldcryptoforum/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/WCFCRYPTO/WCF
...
./bin/gbuild --commit worldcryptoforum=${COMMIT} --url worldcryptoforum=${URL} ../worldcryptoforum/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit worldcryptoforum=${COMMIT} --url worldcryptoforum=${URL} ../worldcryptoforum/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit worldcryptoforum=${COMMIT} --url worldcryptoforum=${URL} ../worldcryptoforum/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/worldcryptoforum-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/worldcryptoforum-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/worldcryptoforum-build.assert
```
...
[worldcryptoforum/gitian.sigs](https://github.com/worldcryptoforum-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
